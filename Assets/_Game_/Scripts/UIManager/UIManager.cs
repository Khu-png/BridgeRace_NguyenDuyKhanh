...
public class UIManager : Singleton<UIManager>
{
    ...
    public bool TryGetUI<T>(out T canvas) where T : UICanvas
    {
        canvas = null;
        System.Type type = typeof(T);
        ...
        if (IsLoaded<T>())
        {
            canvas = uiCanvas[type] as T;
            return canvas != null;
        }

        if (!TryGetUIPrefab(out T prefab))
        {
            return false;
        }

        UICanvas instance = Instantiate(prefab, CanvasParentTF);
        uiCanvas[type] = instance;
        canvas = instance as T;
        return canvas != null;
    }

    public void CloseAll()
    {
        foreach (KeyValuePair<System.Type, UICanvas> item in uiCanvas)
        {
    ...
    private bool TryGetUIPrefab<T>(out T prefab) where T : UICanvas
    {
        prefab = null;
        System.Type type = typeof(T);
        ...
        {
            prefab = uiCanvasPrefab[type] as T;
            return prefab != null;
        }

        for (int i = 0; i < uiResources.Length; i++)
        {
            if (uiResources[i] is T matchedPrefab)
            {
                uiCanvasPrefab[type] = matchedPrefab;
    ...
    {
        if (Keyboard.current != null && Keyboard.current.escapeKey.wasPressedThisFrame && BackTopUI != null)
        {
